...
const ListHeader = React.memo(() => {
  const { newDemarche, setNewDemarche, selectedKeyId } = useContext(SettingContext)
  const handleAddService = useCallback(() => {
    if (!newDemarche && selectedKeyId) {
      setNewDemarche(new CalendarItemType({ name: 'New Demarche', parentId: selectedKeyId, id: v4() }))
    } else {
      // selectedkeyId undefined ? => error
    }
  }, [newDemarche, setNewDemarche])
  return (
    <div className="list-header">
      <div>Services :</div>
      <Tooltip title="Add a new site">
        <Button icon={<PlusOutlined />} disabled={!!newDemarche} onClick={handleAddService} 

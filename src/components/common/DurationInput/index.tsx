...
interface DurationInputProps {
  value?: number | null // Incoming value (total minutes) from Form.Item
  onChange?: (totalMinutes: number | undefined) => void // To update Form.Item
...
export const DurationInput: React.FC<DurationInputProps> = ({
  value,
  onChange,
  defaultUnit = 'days', // Default unit if value is null or for initial state
  placeholder = '0',
}) => {
  ...
  const decomposeMinutesForDisplay = (totalMinutes: number | null | undefined, preferredInitialUnit: string) => {
    ...
    if (unitMultipliers[preferredInitialUnit] && totalMinutes % unitMultipliers[preferredInitialUnit] === 0) {
      return { quantity: totalMinutes / unitMultipliers[preferredInitialUnit], unit: preferredInitialUnit }
    }
    ...
    for (let i = unitOptions.length - 1; i >= 0; i--) {
      ...
      if (totalMinutes >= unitMultipliers[unitInfo.value] && totalMinutes % unitMultipliers[unitInfo.value] === 0) {
    ...
    if (unitMultipliers[preferredInitialUnit]) {
      return { quantity: parseFloat((totalMinutes / unitMultipliers[preferredInitialUnit]).toFixed(2)), unit: preferredInitialUnit }
    }
